...
def test_refresh_round_hashes_is_packaged_and_smoked() -> None:
    shell_sources = scripts_shell_sources()
    pex_targets = workflow_pex_targets()
    cli_sources = cli_python_sources()
    runtime_deps = workflow_runtime_deps()
    tool_name = "refresh-round-hashes"
    module = WORKFLOW_COMMAND_MODULES[tool_name]
    module_name = module.rsplit(".", 1)[-1]
    smoke_text = (REPO_ROOT / "scripts" / "smoke-refresh-round-hashes").read_text(encoding="utf-8")

    assert cli_sources[module_name] == f"{module_name}.py"
    assert f"src/thesis_review_workflow/cli:{module_name}" in runtime_deps
    assert pex_targets[tool_name]["entry_point"] == f"{module}:console_main"
    assert tool_name in shell_sources
    assert "smoke-refresh-round-hashes" in shell_sources
    assert "scripts/refresh-round-hashes" in smoke_text
...
def test_materialize_submission_bundle_candidate_is_packaged_and_smoked() -> None:
    shell_sources = scripts_shell_sources()
    pex_targets = workflow_pex_targets()
    cli_sources = cli_python_sources()
    runtime_deps = workflow_runtime_deps()
    tool_name = "materialize-submission-bundle-candidate"
    module = WORKFLOW_COMMAND_MODULES[tool_name]
    module_name = module.rsplit(".", 1)[-1]
    smoke_text = (REPO_ROOT / "scripts" / "smoke-submission-bundle-materialization").read_text(encoding="utf-8")

    assert cli_sources[module_name] == f"{module_name}.py"
    assert f"src/thesis_review_workflow/cli:{module_name}" in runtime_deps
    assert pex_targets[tool_name]["entry_point"] == f"{module}:console_main"
    assert tool_name in shell_sources
    assert "smoke-submission-bundle-materialization" in shell_sources
    assert "scripts/materialize-submission-bundle-candidate" in smoke_text
...
def test_review_round_closeout_is_packaged_and_smoked() -> None:
    ...
    cli_sources = cli_python_sources()
    runtime_deps = workflow_runtime_deps()
    module = WORKFLOW_COMMAND_MODULES["review-round-closeout"]
    module_name = module.rsplit(".", 1)[-1]
    smoke_text = (REPO_ROOT / "scripts" / "smoke-review-round-closeout").read_text(encoding="utf-8")

    assert cli_sources[module_name] == f"{module_name}.py"
    assert f"src/thesis_review_workflow/cli:{module_name}" in runtime_deps
    assert pex_targets["review-round-closeout"]["entry_point"] == f"{module}:console_main"
    assert "review-round-closeout" in shell_sources
    assert "smoke-review-round-closeout" in shell_sources
    assert "scripts/review-round-closeout" in smoke_text


def test_export_opponent_report_is_packaged_and_smoked() -> None:
    shell_sources = scripts_shell_sources()
    pex_targets = workflow_pex_targets()
    cli_sources = cli_python_sources()
    runtime_deps = workflow_runtime_deps()
    module = WORKFLOW_COMMAND_MODULES["export-opponent-report"]
    module_name = module.rsplit(".", 1)[-1]
    smoke_text = (REPO_ROOT / "scripts" / "smoke-export-opponent-report").read_text(encoding="utf-8")

    assert cli_sources[module_name] == f"{module_name}.py"
    assert f"src/thesis_review_workflow/cli:{module_name}" in runtime_deps
    assert pex_targets["export-opponent-report"]["entry_point"] == f"{module}:console_main"
    assert "export-opponent-report" in shell_sources
    assert "smoke-export-opponent-report" in shell_sources
    assert "scripts/export-opponent-report" in smoke_text
