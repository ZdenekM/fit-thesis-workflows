"""Record a submitted supervisor-report PDF and bind it to reviewed state."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="scripts/record-submitted-supervisor-report",
        description=(
            "Copy the submitted supervisor-report PDF into the ignored round workspace and record "
            "a hash-bound comparison against the reviewed Markdown and supervisor confirmation."
        ),
    )
    parser.add_argument("case_id")
    parser.add_argument("round_id", nargs="?")
    parser.add_argument("--pdf", required=True, help="submitted supervisor report PDF")
    parser.add_argument(
        "--public-text-file",
        default="",
        help="optional extracted public report text; if omitted, pdftotext -layout is used",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        pdf_rel = copy_submitted_pdf(resolve_caller_path(args.pdf), round_dir, force=args.force)
        ...
        errors = validate_submitted_report_record(payload, round_dir=round_dir, case_id=args.case_id, round_id=round_id)
        if errors:
            raise ValueError("\n".join(errors))
        output = round_dir / SUPERVISOR_REPORT_SUBMITTED_RECORD_REL
