...
- Use Serena MCP first for code navigation and symbol-level edits when the language is supported: `get_symbols_overview`, then `find_symbol` with `include_body`, then `find_referencing_symbols`. Reading a source file is for confirming something already located, not for finding it; `rg` finds where, Serena reads what. Long Markdown goes to local RAG instead, and submitted code under `cases/**` is outside Serena's scope. See `docs/serena-code-navigation.md`.
...
- Use the shared `mcp-local-rag` bridge (`http://127.0.0.1:8775/mcp`, one server for every repository on this workstation) as the discovery layer for historical orientation across Markdown-heavy workflow docs, plans, skills, reviewer profiles, notes, and generated Markdown outputs; follow `docs/local-rag-usage.md`. It is also where long Markdown goes instead of Serena.
...
- Never run a local RAG ingest, sync, or delete yourself. The index is machine-level state shared by every session on this workstation, refreshed outside any agent's run by a `systemd --user` timer. If a query returns nothing for content visible on disk, the index is stale for that file: say so and fall back to `rg`. Report it; do not fix it by ingesting.
- `BASE_DIRS` is bridge-level machine configuration, not a per-repo setting and not a file-level allowlist or exclude mechanism. A broad configured root such as `cases/` only makes targeted files reachable; it does not sanction indexing them. Scope questions about it belong to the operator, not to an agent-run ingest.
- The following must not reach the local RAG index: prepared submitted-code workspaces, unpacked submitted-source directories, raw GitHub intake snapshots, and extracted thesis text, especially `cases/**/work/code/**`, `cases/**/work/submission_bundle/**`, raw `cases/**/inputs/github/**` diffs/patches/comments/logs, nested non-evidence `cases/**/inputs/**` source trees, and `cases/**/extracted/**`. Case RAG is for notes, assignments, direct Markdown inputs such as prior feedback, curated GitHub intake summaries, reviewer profiles, generated Markdown outputs, and operator work summaries. If indexed content appears that should not be there, report it to the operator rather than purging or re-ingesting.
- Submitted code stays in the code-review path with `rg`, direct source reads, Omen, and tests. Serena does not cover it: `cases/**` is in `ignored_paths` in the tracked `.serena/project.yml`, and the shared server cannot be repointed at a per-case root.
