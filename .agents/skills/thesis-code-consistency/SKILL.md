...
6. If code is present only as an archive or source directory in `inputs/`, run `scripts/prepare-code-workspace <case-id> [round-id]` when the current task permits writing ignored case workspace evidence. Use `work/code_workspace.md` and `work/serena_roots.json` as orientation aids for which roots exist and where to look; inspect them with `rg` and targeted reads, one root at a time. Serena does not cover submitted code: `cases/**` is in `ignored_paths` in the tracked `.serena/project.yml`, and the shared single-project server cannot be repointed at a per-case root. If you are in a read-only reviewer agent and the prepared workspace is missing, report that limitation instead of silently skipping code checks.
...
After writing or revising `outputs/code_consistency.md`, register the artifact
through the current `work/review_role_plan.json` preset when available, usually
with `scripts/register-review-artifact <case-id> <round-id>
outputs/code_consistency.md --role code_consistency`, including source refs,
...
`scripts/check-code-consistency --require-synthesis-handoff <case-id>
...
low-cost model for the first or only pass over thesis claims, code evidence,
reproducibility, or text-code mismatch findings. Mechanical helper summaries may
