...
9. If code is present only as an archive in `inputs/`, prepare an inspectable copy under `work/code/` before delegating to read-only reviewers. If the code is available through GitHub repo/PR URLs, run `thesis-github-code-intake` first and keep the resulting `outputs/github_code_intake.md` as internal evidence. In an early round with no submitted archive, the round should declare the source with `scripts/review-round-start --profile supervisor_feedback --code-source github <case-id> [round-id]`, or on an already-started round with `scripts/prepare-review-round --code-source github <case-id> [round-id]`; that makes the intake a required next action instead of leaving the round silent about code. Note that a declaration blocks the wave gate and closeout until the intake exists or a typed `github_intake` limitation is accepted. If agent authorization is missing, stop before final output and ask for authorization instead of recording an agent-review limitation.
...
12. Build a short private map:
   ...
   - Theses.cz similarity-report status when an imported report is present,
...
- For Theses.cz similarity-report findings, cite `outputs/theses_similarity_review.md` internally. Do not mention clean or resolved reports in student-facing prose. Do not expose raw report URLs, source internals, hashes, or local paths.
- Treat standalone `outputs/github_code_intake.md`, `outputs/code_consistency.md`, `outputs/code_quality_review.md`, `outputs/literature_citation_review.md`, `outputs/figure_media_review.md`, `outputs/typography_formal_review.md`, and `outputs/theses_similarity_review.md` as internal/operator evidence unless the user explicitly asks to send them.
