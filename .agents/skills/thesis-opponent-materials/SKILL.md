...
7. If a submitted parent ZIP/bundle is present, run or reuse `scripts/review-round-start --profile opponent_materials ... --submission-bundle inputs/<bundle>` so the bundle is both inventoried and expanded under `work/submission_bundle/`. Do not treat a large archive as metadata-only evidence when evaluating assignment fulfillment. For assignment points that require concrete artefacts such as datasets, annotation exports, split manifests, ground truth, raw evaluation outputs, videos, or source code, inspect the expanded workspace before saying the artefact is absent or only weakly documented. If the expansion manifest records skipped or bounded entries, carry that as a review limitation rather than evidence that the missing artefact is absent.
8. If code is present only as an archive in `inputs/`, prepare an inspectable copy under `work/code/` before delegating to read-only reviewers. If the code is available through GitHub repo/PR URLs, run `thesis-github-code-intake` first and keep the resulting `outputs/github_code_intake.md` as internal evidence. If agent authorization is missing, stop before final output and ask for authorization instead of recording an agent-review limitation.
9. When quantitative, evaluation, experiment, metric, performance, or result claims matter to opponent synthesis, an authorized quantitative-claims reviewer agent or human must first write `work/quantitative_claims.json` with evidence anchors, units, scale/sample context, baseline/comparator status, practical magnitude, overclaim risk, reproducibility refs, and limitations. Then run `scripts/check-evaluation-claims <case-id> [round-id]` to validate that structured artifact before synthesis. Do not use deterministic text matching to decide whether such claims exist or what they mean. Text, code, and figure/media agents that discover material prose-only quantitative claims must route them to this skill rather than expanding deterministic raw-text scans.
10. When text/assignment or evidence-calibration packet roles are active, their findings belong in the role-owned sidecars named by `work/review_role_plan.json` (`work/opponent_packets/text_structure_assignment_findings.md` and `work/opponent_packets/evidence_calibration_findings.md`). Do not create parent-authored filler files for those roles. For clean Theses.cz no-concern evidence, use the validated `work/theses_similarity/assessment.json` plus reviewed synthesis manifest coverage with `silent_internal_evidence:no_material_concern`; do not require `outputs/theses_similarity_review.md` unless the assessment records a non-silent or reviewer-verification concern.
11. Before spawning role-split agents, use the optimized deterministic boundary. If a submitted parent ZIP/bundle is present, this is the same bundle-aware `review-round-start` invocation from step 7, including all current `--submission-bundle`, `--bundle-classification`, and `--bundle-child` flags; do not rerun the generic command without those flags because that would replace the start trace with an incomplete material view:
...
15. Run `thesis-literature-citation-review` when literature relevance, citation support, or source defensibility is material to the opponent assessment. For opponent work, use it only for relevance, defensibility, citation quality, and support for submitted claims; do not turn it into literature coaching.
16. Run `thesis-typography-formal-review` when the thesis is in final submission state or formal presentation may affect report quality. Use it as pattern evidence; do not turn it into a long typo inventory.
17. Run `thesis-theses-similarity-review` when an imported Theses.cz report is present. Keep no-concern or resolved matches internal; surface only reviewed unresolved/material concerns or manual checks, and do not expose raw report URLs/source internals.
18. Calibrate severity. Do not search for faults at any cost; identify strong parts with equal care. Distinguish an unsupported or overstated thesis claim from a serious defect in the work itself:
...
19. Use `docs/fit-is-rubric.md` as the shared checklist for FIT IS item coverage.
20. Apply `docs/opponent-review-workflow.md#report-quality-controls` before
...
21. Use confidence labels for important statements:
...
workflow. Run the parent/orchestrator session on the strongest available model at high reasoning effort (`xhigh`); see `docs/agent-scheduling.md`. This work is parent-orchestrated, not a spawned provider adapter. Packet prompts generated for this skill must carry the
