...
17. If Theses.cz similarity-report evidence was used, verify that claims are backed by `outputs/theses_similarity_review.md` or by a validated `work/theses_similarity/assessment.json` that is covered in the reviewed synthesis manifest with `silent_internal_evidence:no_material_concern`. No-concern or resolved reports stay silent, and raw report URLs/source internals/hashes are absent from opponent-facing prose. Reject plagiarism/authorship wording unless a reviewed unresolved concern supports that narrow statement.
