...
| `thesis-opponent-report-review` | `profile` | `thesis_opponent_report_reviewer` | final-reviewer | workspace-write | `outputs/feedback_k_posudku.md`, `work/reviews/opponent_report_review.json` | fresh review is required after material rewrites; review basis is normally `outputs/oponent_posudek_navrh.md` | `scripts/check-review-wave --workflow opponent_report --wave draft`, `scripts/check-opponent-report --mode canonical`, `scripts/export-opponent-report`, `scripts/check-opponent-report --mode clean` |
